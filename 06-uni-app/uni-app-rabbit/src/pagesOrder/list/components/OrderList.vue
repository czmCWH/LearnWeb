...
// 注意注意注意：在 uni-app 中页面的路由参数和子组件接收的参数都是通过 defineProps 接收的
// 页面参数取名：query；组件参数为：props
...
const getMemberOrderData = async () => {
  const res = await getMemberOrderAPI(queryParmas)
...
<scroll-view scroll-y class="orders">
  <view class="card" v-for="order in orderList" :key="order.id">
    ...
    >
      ...
      <view class="meta">
        <view class="name ellipsis">{{ item.image }}</view>
    ...
    <view class="action">
      ...
      <template v-else>
        ...
        <!-- 待收货状态: 展示确认收货 -->
        <view @tap="" v-if="order.orderState === OrderState.DaiShouHuo" class="button primary">确认收货</view>
      </template>
    </view>
  </view>
  <!-- 底部提示文字 -->
