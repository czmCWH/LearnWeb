...
const onOrderPay = async () => {
  ...
  if (import.meta.env.DEV) {  // 开发环境，模拟支付
    const res = await getPayMockAPI({ orderId: query.id })
    console.log('--- 模拟支付=', res.result)
  } else {
...
const onOrderSend = async () => {
  if (isDev) {    // 仅在开发环境下使用，打包到生产环境会剔除以下代码（tree shaking 树摇优化)
...
const onOrderConfirm = () => {
  const res = putMemberOrderReceiptByIdAPI(query.id)
  // 二次确认弹窗
  uni.showModal({
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if ( success.confirm ) {
        ...
        // 进入订单列表
        uni.redirectTo({ url: 'pagesOrder/lis/list' })
      }
    }
  })
}
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="order">
      ...
      <view class="detail">
        ...
        <view class="row">
          <view class="item">
            订单编号: {{ query.id }} <text class="copy" @tap="$event => onCopy(query.id)">复制</text>
          </view>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-if="order.orderState === OrderState.DaiFuKuan">
          ...
          <view class="button" @tap="$event => popup?.open?.()"> 取消订单 </view>
        </template>
        <!-- 其他订单状态:按需展示按钮 -->
        <template v-else>
          <navigator
